...
var r = require("fs").readFileSync("api/data.json",'utf-8')
console.log(r)
...
function render(action, elements) {
  if (typeof window !== "undefined") {
    window.lib = {
      actives: ["home", "cards", "cards1", "cards2", "pricing"],
      current: [],
      image: [],
      imagepart: [],
      id: 0,
      menu: function menu(event) {
        ...
        if (e == "publish") {
          ...
          document.getElementById("registerm").onclick = function() {
            lib.login("publish");
          };
        }
  ...
  window.URL.revokeObjectURL(url);*/
      ...
      login: function login(e, a, o, u, s, b) {
        if (e == "build") {
          ...
          var register_description = document.getElementById("description-name")
            .value;
    ...
    lib.control = function(e) {
      ...
      }
      {
         document.addEventListener("click", function() {
          ...
          if (target == "dashboard") {
            for (var temp of lib.actives) {
              var tempted = document.getElementById(temp);
              console.log(tempted);
            }
          }
...
modalContent.innerHTML = "<div>h</div>"
  modal.style.display = "block";

});


modalContent.innerHTML = "<div><h1>Welcome To Fastur</h1><p>Fastur helps you build a Stripe site for your product or service.</p><h3>PayPal & Stripe</h3><p> Connect your PayPal and Stripe account to any marketing campaign. </p><h3>Event Ticket Sales</h3><p> Sell tickets and gather admission fees for physical or virtual events.</p><h3> Zero Fee Donations</h3><p> Get donations without having to pay 20% to fundraising platforms.</p><h3> Online sales </h2><p>Generate sales online automatically with two-click mobile payments.</p></div>"
  modal.style.display = "block";
    ...
    if (action == "edit") {
      elements.push(
        ...
        },
        {
          ...
          href: "#",
          style:
            "padding: 15px; font-size: 17px; width:275px; border: none; background: white;",
          datadisplay: "uform",
          position: "fixed",
          right: "10px",
          top: "calc( 100vh - 80px )",
  ...
  for (var element in elements) {
    ...
    {
  css += ` ${classReady || "card"} { text-align: ${b.align || "center"}; background: ${b.background || "#FFFFFF"}; border-radius: ${b.rounding || "10px"}; outline: ${b.outline || "dashed"}; draggable: ${b.isdrag || "true"}; font-family: ${b.font || "Arial, Helvetica, sans-serif;"}; text-decoration: ${b.decoration || "none"}; transition: ${b.transition || "all 0.5s ease-out;"}; opacity: ${b.opacity || "1;"}; color: ${b.color || ""}; margin: ${b.margin || "10 auto;"}; position: ${b.position || "relative;"}; left: ${b.left || ""}; right: ${b.right || ""}; width: ${b.width || ""}; height: ${b.height || ""}; bottom: ${b.bottom || ""}; top: ${b.top || ""}; padding: ${b.padding || ""}; overflow: ${b.overflow || ""}; z-index: ${b.z || ""}; display: ${b.display || "inline-block"}; font-size: ${b.size || " 1rem; "}; flex: ${b.flex || ""}; border: ${b.border || ""}; box-sizing: ${b.box || ""}; float: ${b.float|| ""}; }
...
`;
  ...
  for (var a = 0; a < containers.length; a++) {
    var containersTemp = "<div id='"+containers[a].id+"' style='display:flex;' class='"+containers[a].class+"'>"
    for (var i = 0; i < parents.length; i++) {
    
      var parentsTemp = "<" + (parents[i].tag || "div") + " href='" + parents[i].href + "' style='" + (parents[i].style || "padding: 0px; margin:auto;") + "' id='" + (parents[i].id || "card") + "' onclick=" + (parents[i].onclick || "") + " src='" + (parents[i].src || "") + "' contenteditable='" + (parents[i].contenteditable || " ") + "' type='" + (parents[i].formtype || "") + "' value='" + (parents[i].value || "") + "' data-display='" + (parents[i].datadisplay || "") + "' data-link='" + (parents[i].datalink || "") + "' placeholder='" + (parents[i].placeholder || "") + "' class='" + (parents[i].class || parents[i].animation || "card") + "' data-action='" + (parents[i].dataid || "") + "' > ";
    
    for (var k = 0; k < children.length; k++) {
      if (children[k].datadisplay == parents[i].id) {
        
  parentsTemp += "<" + (children[k].tag || "div") + " href='" + children[k].href + "' style='" + (children[k].style || "padding: 0px; margin:auto;") + "' id='" + (children[k].id || "card") + "' onclick=" + (children[k].onclick || "") + " src='" + (children[k].src || "") + "' contenteditable='" + (children[k].contenteditable || " ") + "' accept='"+(children[k].accept || "")+"' type='" + (children[k].formtype || "") + "' value='" + (children[k].value || "") + "' data-display='" + (children[k].datadisplay || "") + "' data-link='" + (children[k].datalink || "") + "' placeholder='" + (children[k].placeholder || "") + "' class='" + (children[k].class || children[k].animation || "card") + "' data-action='" + (children[k].dataid || "") + "' > " + (children[k].name || "") + "<" + (children[k].close || "/") + (children[k].tag || "div") + ">";
    ...
    temp += containersTemp;
     ...
     temp +=
      "<div id='myModal' class='modal'><div class='modal-content'> <span class='close'>&times;</span> <p id='modalContent'></p> </div></div><div> <a id='support' class='carrd_bttn' href='#support'> <img class='carrd_bttn_img' src='https://image.flaticon.com/icons/svg/561/561127.svg' alt='carrd_bttn_img'> <div class='carrd_bttn_text'>Support</div> </a> </div>" ;
  ...
  if (elements) {
    var html =
      ...
      "<style>" +
      css +
      "</style><link type='image/png' rel='shortcut icon' href='api/ico.png'></head><body id='"+(elements[0].page || "")+"' style='background-attachment: fixed; background-position: center; background-repeat: no-repeat; background-size: cover;background-color:" +
    ...
    if (request.method == "GET") {
     ...
     if (request.url == "/login") {
      ...
      if (request.url == "/") {
        ...
        var data = require("fs").readFileSync("./api/93e4e3670e51064dcc1247c9c3301e50.json");

        var data = JSON.parse(data);
        var html = render("false", data);
        response.end(html);
      } else 
        if (require("fs").existsSync(__dirname + "/" + request.url)) {
          var img = require("fs").readFileSync(__dirname + "/" + request.url) || null;response.end(img);
   }
              ...
              case "publish": {
                ...
                if (object.modifier == "editor") {
                  ...
                  var uid = require("crypto")
                    .randomBytes(16)
                    .toString("hex");
                  //var query = JSON.parse(object.elements);
                  if (process.env.PROJECT_DOMAIN == "fastur") {
                    var html = render("false", object.elements);
                  ...
                  var responseFinal = "successfully published to <a href='https://mypaymentpage.com/"+object.name+"'>mypaymentpage.com/"+object.name+"</a>";
                  var obj = {type:'success',reason:responseFinal}
...
console.log(object.expiry.split('/')[0])
                ...
                stripe.paymentMethods.attach(paymentid,{ customer: result.stripeid },
                  function(err, paymentMethod) {
              ...
              case "landing": {
                function landing(uuid) {
                  ...
                  if (result){
                  var string = JSON.stringify({"type":"success","data":result});
                  response.end(string)
                  } else {
                    
                  var string = JSON.stringify({"type":"failed","reason":"not found"});
                  response.end(string)
                  }
                }
