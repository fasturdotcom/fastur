(function() {
  ...
  Game.prototype = {
    ...
    draw: function(screen, gameSize) {
      ...
      screen.fillText(this.bodies.length, 15, 10);
      screen.fillText(score, 1, 10);
      screen.fillText(gamestate, 35, 10);
      ...
      for (var i = 0; i < this.bodies.length; i++) {
        ...
        if (this.bodies[i].type == "invaders") {
          count++;
        }
        if (count == 0) {
          score = score + 100;
...
var Q_table = {}; 
var actions = { left: 0, right: 1, nothing: 2, shoot: 3 };
var actionList = ["left","right","nothing","shoot"]
...
//execute action in environment and get the reward as a float
console.log(Q_table)
  var spaceInvaders = document.getElementById("space-invaders");
  var context = spaceInvaders.getContext("2d");
  var data = context.getImageData(0, 0, 8, 8).data;

  //initialize two hidden layers with random values
  //might need to initialize 4 layerss, one for each action
  var w1 = Array.from({ length: 256 },() => Math.floor(Math.random() * 255) / 255);
  var w2 = Array.from({ length: 256 },() => Math.floor(Math.random() * 255) / 255);

  console.log(actions)
  var action = actionList[Math.floor(Math.random() * actionList.length)];

  setQ(data, action, lib.score);  
  var a = getQ(data,'left')
  var b = getQ(data,'right')
  var c = getQ(data,'nothing')
  var d = getQ(data,'shoot')

  actions.left = a;
  actions.right = b;
  actions.nothing = c;
  actions.shoot = d;
var action = Object.keys(actions).reduce(function(a, b) {
    return actions[a] > actions[b] ? a : b;
  });
  console.log(action)
  var node = 0;
  ...
  // rectified linear unit, return positive numbers / squash negative numbers as 0
  function relu(a) {
    if (a > 0) {
      return a;
    } else {
  ...
  //sigmoid function (gives probability of going up)
  function sigmoid(a) {
    return 1/(1+Math.pow(Math.E, -a));
}
  
  var node = relu(node);
  var node = sigmoid(node);
  
  actions.left = node;
  actions.right = node;
  actions.nothing = node;
  actions.shoot = node;
